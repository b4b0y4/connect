  ...
  #incrementBytesRead(count) {
    ...
    assert(
      ...
      "BUFFER_OVERRUN",
      {
        ...
        offset: this.#offset,
        length: count,
        info: { bytesRead: this.#bytesRead, dataLength: this.dataLength },
      }
    )
  }
    ...
    .map(({ input, error, output }) => {
      if (error) {
        ...
        throw new Error(
          split.length == 1
            ? msg
            : `Invalid label ${bidi_qq(safe_str_from_cps(input, 63))}: ${msg}`
        )
      }
...
function consumeEoi(tokens) {
  if (tokens.length) {
    throw new Error(
      `unexpected tokens at offset ${tokens.offset}: ${tokens.toString()}`
    )
  }
}
      ...
      const broadcasts = this.#configs.map(
        async ({ provider, weight }, index) => {
          try {
            ...
            results[index] = Object.assign(normalizeResult({ error: error }), {
              weight: weight,
            })
          }
        }
